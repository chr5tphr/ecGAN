...
save_aligned_image(data=gen, fpath='/home/chrstphr/gen/gen%04d.png'%100,bbox=config.data.bbox)

cond = nd.concat(nd.ones((30,2,1,1),ctx=ctx)/2.,nd.zeros((30,8,1,1),ctx=ctx),dim=1)
gen = model.generate(noise=noise, cond=cond)
save_aligned_image(data=gen, fpath='/home/chrstphr/gen/gen%04d.png'%101,bbox=config.data.bbox)
...
for i in range(10):
    ...
    gen = model.generate(noise=noise, cond=cond)
    save_aligned_image(data=gen, fpath='/home/chrstphr/gen/gen%04d.png'%i,bbox=config.data.bbox)

# random walk through latent space
bbox = config.data.bbox
anoi = nd.random.normal(shape=(30,100,1,1), ctx=ctx)
bnoi = nd.random.normal(shape=(30,100,1,1), ctx=ctx)
slast = 0.
stepsize = 0.1
momentum = 0.9
with imageio.get_writer('/home/chrstphr/gen/latentwalk.mp4', macro_block_size=28, quality=5., fps=24, ffmpeg_params=['-sws_flags','neighbor','-s', '%dx%d'%(4*6*28,4*5*28)]) as writer:
